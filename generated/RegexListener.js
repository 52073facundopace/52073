// Generated from c:/Users/Ulises/regex-analyzer/Regex.g4 by ANTLR 4.13.2
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by RegexParser.
export default class RegexListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by RegexParser#regex.
	enterRegex(ctx) {
	}

	// Exit a parse tree produced by RegexParser#regex.
	exitRegex(ctx) {
	}


	// Enter a parse tree produced by RegexParser#term.
	enterTerm(ctx) {
	}

	// Exit a parse tree produced by RegexParser#term.
	exitTerm(ctx) {
	}


	// Enter a parse tree produced by RegexParser#factor.
	enterFactor(ctx) {
	}

	// Exit a parse tree produced by RegexParser#factor.
	exitFactor(ctx) {
	}


	// Enter a parse tree produced by RegexParser#CharBase.
	enterCharBase(ctx) {
	}

	// Exit a parse tree produced by RegexParser#CharBase.
	exitCharBase(ctx) {
	}


	// Enter a parse tree produced by RegexParser#GroupBase.
	enterGroupBase(ctx) {
	}

	// Exit a parse tree produced by RegexParser#GroupBase.
	exitGroupBase(ctx) {
	}


	// Enter a parse tree produced by RegexParser#ClassBase.
	enterClassBase(ctx) {
	}

	// Exit a parse tree produced by RegexParser#ClassBase.
	exitClassBase(ctx) {
	}


	// Enter a parse tree produced by RegexParser#group.
	enterGroup(ctx) {
	}

	// Exit a parse tree produced by RegexParser#group.
	exitGroup(ctx) {
	}


	// Enter a parse tree produced by RegexParser#characterClass.
	enterCharacterClass(ctx) {
	}

	// Exit a parse tree produced by RegexParser#characterClass.
	exitCharacterClass(ctx) {
	}


	// Enter a parse tree produced by RegexParser#range.
	enterRange(ctx) {
	}

	// Exit a parse tree produced by RegexParser#range.
	exitRange(ctx) {
	}


	// Enter a parse tree produced by RegexParser#quantifier.
	enterQuantifier(ctx) {
	}

	// Exit a parse tree produced by RegexParser#quantifier.
	exitQuantifier(ctx) {
	}



}