// Generated from c:/Users/Ulises/regex-analyzer/Regex.g4 by ANTLR 4.13.2
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete generic visitor for a parse tree produced by RegexParser.

export default class RegexVisitor extends antlr4.tree.ParseTreeVisitor {

	// Visit a parse tree produced by RegexParser#regex.
	visitRegex(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by RegexParser#term.
	visitTerm(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by RegexParser#factor.
	visitFactor(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by RegexParser#CharBase.
	visitCharBase(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by RegexParser#GroupBase.
	visitGroupBase(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by RegexParser#ClassBase.
	visitClassBase(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by RegexParser#group.
	visitGroup(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by RegexParser#characterClass.
	visitCharacterClass(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by RegexParser#range.
	visitRange(ctx) {
	  return this.visitChildren(ctx);
	}


	// Visit a parse tree produced by RegexParser#quantifier.
	visitQuantifier(ctx) {
	  return this.visitChildren(ctx);
	}



}